...
-summarize methods from abstract

-talk about last week's bioinformatics methods
...
find what is most common between male and female
```{r common between seses}
...
```{r summarize-length}
#this code makes finds longest sequences on male and female hands
joined_blast_data_metadata %>%
  group_by(sscinames, sex_s) %>%
  summarize(mean_seq_length = mean(length)) %>%
  arrange(desc(mean_seq_length)) %>%
  head(20) %>%
  kable()
...
ggplot(joined_blast_data_metadata, aes(x = length,
                                       fill = sample_type_s)) + 
  geom_histogram() + 
